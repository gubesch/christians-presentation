...
function ComponentsResolver(options = {}) {
  ...
  return (name) => {
    ...
    if (name.includes(":")) {
      ...
      collection = _strings.camelToKebab.call(void 0, iconPrefix);
      ...
      icon = _strings.camelToKebab.call(void 0, iconSuffix);
    } else {
      const kebab = _strings.camelToKebab.call(void 0, name);
      if (!kebab.startsWith(prefix))
        return;
      const slice = kebab.slice(prefix.length);
      const resolvedCollection2 = collections.find((i) => slice.startsWith(`${i}-`)) || collections.find((i) => slice.startsWith(i));
      if (!resolvedCollection2)
        return;
      collection = resolvedCollection2;
